...
function toCamelCase(str) {
  ...
  }).replace(/\s+/g, '').replace(/&/g, 'And').replace(/-/g, '');
}
...
items.forEach((item, index) => {
  ...
  const varName = toCamelCase(item.name);
  const fileName = toFileName(item.name);
  
  importsCode += `import ${varName} from '../assets/images/${fileName}';\n`;
  
  const ratingStr = item.rating ? `, rating: ${item.rating}` : '';
  arrayCode += `    { id: ${item.id}, name: "${item.name}", price: ${item.price}, category: "${item.category}"${ratingStr}, image: ${varName} }${index === items.length - 1 ? '' : ','}\n`;
});

arrayCode += '  ];';

const fs = require('fs');
fs.writeFileSync('generated_menu_code.txt', importsCode + '\n' + arrayCode);
